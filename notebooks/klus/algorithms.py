...
def kedmd(X, Y, k, epsilon=0, evs=5, operator='P',kind='kernel',cond=1e-8):
    ...
    if kind =='kernel':
        A = _sp.linalg.pinv(G_0, rcond=cond) @ G_1        #rcond->sostituito con rtol          #G_0 + epsilon*_sp.eye(n)-->sostituito con G_0 
    elif kind == 'embedded':
         A = G_1 @_sp.linalg.pinv(G_0, rcond=cond)    #rcond->sostituito con rtol          #G_0 + epsilon*_sp.eye(n)-->sostituito con G_0 
    else:
    ...
    if operator == 'K':
        ...
        C = _sp.linalg.pinv(G_0, rcond=cond) @ V    #rcond->sostituito con rtol          #G_0 + epsilon*_sp.eye(n)-->sostituito con G_0 
